...
q_resamples <- qscores %>% 
  ...
  ggplot(q_resamples, aes(x = mean_hours, y = after_stat(count/sum(count)))) +
  geom_histogram(binwidth = .01, fill = "dodgerblue", color = "white") +
    theme_classic() +
    labs(x = "Mean Hour", 
         y = "Probability",
         title = "Posterior Distribution for the Mean Hour Students Spend per Week on Harvard Courses")
